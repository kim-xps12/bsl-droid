...
def main():
    ...
    # 準備: 応答のあるノードを IDLE にし、位置制御モード・closed loop へ移行
        ...
        # ここでctrl+cが押されるまで待機
        try:
            print("Stand: entering idle wait. Press Ctrl+C to exit and return nodes to IDLE.")
            # 安全に待つループ。Ctrl+C（KeyboardInterrupt）で抜ける。
            while True:
                time.sleep(1.0)
        except KeyboardInterrupt:
            print("Stand: Ctrl+C received, proceeding to cleanup...")
