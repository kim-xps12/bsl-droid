...
# 制御ループ用グローバル
control_targets = []  # list of dicts: {'node_id': int, 'origin': float}
control_timer = None
GEAR_RATIO = 8.0
TARGET_POS_DEGREE = -30.0  # 出力軸での回転量(度)
...
def control_update(control_target, target_degree):

    print(control_target)
    """定期的に呼ばれる制御更新関数"""
    # ここに位置制御ループの中身を実装
    # 制御周期に基づいて分割した微小なズレの目標位置を送信する
    # control_targets に設定された各ノードの origin(ターン) を基準に +10度(出力軸)分を送信する
    # global control_targets
    # 出力軸 10度 -> モーター回転(turn) 変換

    # print(control_targets)
    # print(target_degree)

    # for t in control_target:
    output_turns = target_degree / 360.0
    motor_turns = output_turns * GEAR_RATIO

    node_id = control_target['node_id']
    origin_turn = control_target.get('origin', 0.0)
    target_turn = origin_turn + motor_turns
    try:
        # 速度/トルクフィードフォワードは 0
        set_position(node_id, target_turn, vel_ff=0, torque_ff=0)
    except Exception as e:
        print(f"control_update: failed to send to Node {node_id}: {e}")
...
def main():
    ...
    try:
        # まず接続確認を行い、応答があるノードのリストを作成
        responsive_nodes = []
        for node_id_str, data in motors.items():
            node_id = int(node_id_str)
            print(f"Checking connectivity for Node {node_id}...")
            if is_node_connected(node_id, timeout=0.5):
                responsive_nodes.append((node_id, data['shadow_count']))
            else:
                print(f"Node {node_id} not connected or not responding. Skipping.")

        if not responsive_nodes:
            print("No responsive nodes found. Exiting.")
            return

        # 準備: 応答のあるノードを IDLE にし、位置制御モード・closed loop へ移行
        print(f"Preparing {len(responsive_nodes)} node(s) for synchronous move...")
        for node_id, shadow_count in responsive_nodes:
            clear_errors(node_id)
            time.sleep(0.05)
            set_axis_state(node_id, AXIS_STATE_IDLE)
            time.sleep(0.05)
        ...
        # 一斉送信: 各ノードの target_position を計算して同時に送信
        print("Sending synchronous position commands...")
        msgs = []
        for node_id, shadow_count in responsive_nodes:
            # 補正があれば適用
            corrected_shadow = shadow_count
            if zero_offsets and isinstance(zero_offsets, dict):
                z = zero_offsets.get(node_id)
                if z is not None:
                    corrected_shadow = shadow_count - z

            target_position = corrected_shadow / 16384

            # control_targets に初期基準位置（turn単位）を保存
            control_targets.append({'node_id': node_id, 'origin': target_position})

            # 位置コマンドのデータを作成（struct '<fhh' と同じ形）
            vel_ff_scaled = int(0 * 1000)
            torque_ff_scaled = int(0 * 1000)
            data = struct.pack('<fhh', target_position, vel_ff_scaled, torque_ff_scaled)
            can_id_val = (node_id << 5) + CMD_ID_SET_INPUT_POS
            msgs.append(can.Message(arbitration_id=can_id_val, data=data, is_extended_id=False))
        ...
        for node_id, shadow_count in responsive_nodes:
            final_shadow = get_current_shadow_count(node_id)
            print(f"Node {node_id} final shadow_count: {final_shadow}")
        ...
        # グローバルの control_timer に保存して後で停止できるようにする
        ...
        for control_target in control_targets:
            node_id = control_target['node_id']
            shadow_count = get_current_shadow_count(node_id)

            goal = -90.0  # 出力軸での回転量(度)
            max_steps = 300  # 3秒で移動

            start_degree = 0# (shadow_count / 16384*GEAR_RATIO)  # 現在の出力軸角度(度)
            print(f"now degree: {start_degree}")
            
            dx = (goal - start_degree) / max_steps

            pos = 0  # 位置制御に渡す目標角度．制御ループの中で累積させるので初期値は必ずゼロ
            for _ in range(max_steps):
                pos += dx
                control_update(control_target, pos)
                time.sleep(update_interval)

            shadow_count = get_current_shadow_count(node_id)
            corrected_shadow = shadow_count + (control_target['origin']*16384)
            corrected_turns = corrected_shadow / float(16384*GEAR_RATIO)
            final_degree = corrected_turns * 360.0
            print(f"shadow_count: {shadow_count}, origin: {control_target['origin']*16384}, corrected: {corrected_shadow}, turns: {corrected_turns}")
        ...
        try:
            ...
            pass
        # ユーザ割り込み時は、準備した（応答のあった）ノードのみ IDLE に戻す
        try:
            for node_id, _ in responsive_nodes:
        ...
        # finally でも念のため、実際に操作したノードだけを IDLE に戻す
        try:
            for node_id, _ in responsive_nodes:
                set_axis_state(node_id, AXIS_STATE_IDLE)
            print("Returned prepared nodes to IDLE state")
        except Exception:
            pass
