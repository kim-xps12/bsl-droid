...
class ToeTrajectoryController:
    ...
    def generate_toe_trajectory(self, t):
        ...
        z_amplitude = (self.z_max - self.z_min) / 2
        z = z_center + z_amplitude * np.sin(2 * np.pi * t / self.period)
    ...
    def run(self):
        ...
        print("つま先軌跡制御プロセス開始 (50Hz)")
        print(f"Z軸範囲: {-self.z_max:.2f}m ~ {-self.z_min:.2f}m (胴体中心からの高さ)")
        ...
        try:
            while True:
                ...
                target_pos_right = self.generate_toe_trajectory(t) #[target_pos_left[0], -target_pos_left[1], target_pos_left[2]]
